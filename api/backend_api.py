...
class BackendAPIClient:
    ...
    async def get_network_status(self) -> Dict:
        ...
        Fetches real bioregion count from the bioregion service.
        
        Returns:
            Dict with network status including real bioregion count
        """
        status = await self._cached_get("/api/v1/network-status", ttl=15)
        
        # Enhance with real bioregion count if available
        try:
            bioregion_count = await self.get_bioregion_count()
            status['bioregions_count'] = bioregion_count
            status['data_source'] = 'real'
        except Exception as e:
            logger.warning(f"Could not fetch real bioregion count: {e}")
            # Keep the status as-is with mock data
            if 'data_source' not in status:
                status['data_source'] = 'mock'
    ...
    async def get_bioregion_count(self) -> int:
        ...
        Returns:
            Number of active bioregions
        """
        try:
            data = await self._cached_get("/api/v1/bioregions", ttl=60)
